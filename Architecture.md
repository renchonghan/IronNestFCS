...
#### 渲染线程 (独立循环)
- 持续读各数据源画沙盘: GC push 的实时弹道数据 (弹种 -1 不渲染) / FC 的打击队列信息 (队列/齐射/杀伤圈/预瞄) / 炮弹落点指示器 (GC 击发确认 + 飞行期倒计时传导) / 实体图标
...
- 收尾: 击发确认 = 炮表倒计时启动 (FlyRemaining 从 NaN 变有效, 齐射等两炮) → 完成队列 + 槽位释放 (落点指示由 GC 自检接管, FC 不插手); 目标阵亡 (SRC 消失/令牌离图) → 撤任务; GC 报 FALL → 换炮/重试/换弹种
- Pause 冻结语义: 冻结火控派发与炮塔控制, 显控 (DC) 不冻结; 在途动作继续执行完; 冻结后自动跟踪停止, 但**豁免** (基础福利, 任何状态都不冻结): GC 的炮弹飞行传导 (击发后每帧读游戏倒计时 push DC, 落点指示/计时照跑)
...
## 代码与设计差异 (2026-08-19 核对)

设计在前、代码在后, 下列差异按"未实现 / 实现偏离 / 代码先行(文档待补)"三档列出. 改代码还是改文档按条目注明倾向.
...
2. **智能派发只做了实装匹配**: 乱序执行、前瞻保装药、就绪时间轴估算、"两炮都不匹配时比改造成本 vs 等空闲" 均未实现 (Dispatch 注释明示"乱序/前瞻后续版本"); `FireTask.Priority` 字段存在但派发不使用 — 优先级只在扫荡生成请求时排序 (FDC > 炮兵 > 装甲 > 其他), 队首挑选仍按实装匹配.
3. **预瞄偏移 (leadOffset)**: 设计打击队列指示器输入含"预瞄偏移", 代码 UpdateQueueIndicator 有参数但未用 — 绿色任务虚线连实体当前点, 不提前量.

### 实现偏离 (代码与设计描述不符, 倾向改文档)

4. **AzimuthSelect 形态**: 设计写"选择水平追踪对象 (0/L 1/R)" (FC 输出单选), 实现是每炮一个 bool, FC 侧计算 — 同目标双选 / 单任务选中 / 双任务不同目标时**编号小者优先** (乱序 = 重分配编号, 转向权随之走). 编号优先规则未写进文档.
5. ~~**实时弹道指示器数据源**: 设计写 "GC 每帧 push {炮位, 瞄准点, 杀伤圈, 弹种}", 实际瞄准点+杀伤圈由 FC push (OnAimChanged), GC 只 push 弹种就绪态 (OnBallisticPush). 落点指示器已按 GC 传导重写 (见 MSG 链), 绿十字数据源两分.~~ **已解决 (2026-08-19)**: GC 全权接管 — 瞄准点 (游戏落点标记) / 杀伤圈 (DesiredShell) / 弹种 (CANFIRE 门) / AllReady / 飞时 全部由 GC Loop 每帧 push, FC 的 OnAimChanged 已删; GC 读膛内弹种不需要 FC 传.
6. **HUD 2-5 COFM 相位还在**: 设计写 "原 COFM 2-5 被 AllReady 内部吸收", 实际 LoadRoutine 以 弹对+药对+ReloadStateAtOrAfter("BreachLocked") 判装填完成, HUD 装填期读游戏真实状态码 (BreachLocked → 2-5 COFM, 一闪而过); 装填完成判据是实装匹配而非 AllReady (CanFire 含保险不可用, 保险 TRAK 段才解).
7. **KernelMode 语义弱化**: 设计"期间 FC 指令只记录不生效", 实现无记录机制 — 指令是持续覆盖端口, 动作例程在开工时快照消费 (任务内值不变), 常驻方位追踪不受 KernelMode 影响 (从派发全程追, 这正是原则段"方位追踪不受装弹影响"的体现).

### 代码先行 (代码已改, 文档还没写 — 本次核对补记)

8. **AllReady 的未选中炮 H 检查**: 未选中炮 (AzimuthSelect=false) 的 H 不再"不算" — 等炮塔转到本炮目标方位 (H 轴死区 0.1° 内) 才算稳, 否则两炮不同任务时未选中炮 H 没对准也报 AllReady, FC 会打飞.
9. **统一火控的确认前置**: 火控卡 + 五步确认前置到进 TRAK 时 (瞄准过程中, 不等稳定), AllReady 后只剩解保险 + 击发 — 稳定到开火 ~1.3s (原 5.2s). 文档"五步确认 + Arm + 击发"段未反映; confirmed 置位等协程真跑完 (CoroutineLock 非 FIFO, 不靠锁排队保序), confirming/confirmed 双标志防重入.
10. **TRAK 开头机构等待**: RunTrak 开头等 WaitForReloadReady (机构停 + 炮闩就绪 + 炮管运动停止) + 0.3s 缓冲 — 装填完炮管有回落动作, 追早了对摇杆打架 (鬼畜); 不能等 CanFire (含保险).
11. **SHRD 前残留实装清零等待**: 空膛转弹仓前等 LoadedPowderCharges() 清零 (上一发自动循环; 1.x 同款) — 炮闩打开后机构还要 ~3s 复位, 计数清零才算复位完.
12. **Phantom Battery 排除只存在旧 TacticalRadar**: 新 Radar.cs 只排除阵亡实体 (击杀令牌), Phantom Battery (演示幻影炮组) 排除未搬.
13. **齐射同步区细化**: 设计只说"相位级同步由 GC 互相等" — 实现为 SalvoDirector: 预处理 (1-x SELC/DUMP) 各炮独立推进, 同步区从 2-1 SHRD 起逐步汇合 + 每步 0.5s 死区; 每步双炮跑独立子协程, 导演用完成标志汇合 (Il2Cpp 反射拿不到 WaitForSeconds 秒数, 手动枚举器驱动已弃).
